...
export const jobPost = (job: Job, index: number) => {
  return /*HTML*/ `<div class="flex flex-col items-center justify-center overflow-hidden pt-2 sm:py-10">
          ...
          <div id="jobContent${index}" class="bg-white shadow-sm w-full max-w-4xl flex flex-col sm:flex-row gap-3 sm:items-center  justify-between px-5 py-4 rounded-3xl">
            ...
            ${
              job.email !== removeQuote(myDetails.myEmail)
                ? `<button id='job-apply-btn${index}' data-job-id="${job.jobsTableId}" class="job-apply-btn bg-black text-white font-medium px-4 py-2 rounded-md flex gap-1 items-center font-primary">
                   Apply Now 
                   <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                     <path stroke-linecap="round" stroke-linejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                   </svg>
                 </button>`
                : `<button id='delete-job-btn${index}' data-job-id="${job.jobsTableId}" class="delete-job-btn bg-black text-white font-medium px-4 py-2 rounded-md flex gap-1 items-center font-primary">
                   Delete Now 
                   <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                     <path stroke-linecap="round" stroke-linejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                   </svg>
